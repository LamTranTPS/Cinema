(function (app) {
    ...
    clotheListController.$inject = ["$scope", "apiService", "notifyService", "$ngBootbox"];
    function clotheListController($scope, apiService, notifyService, $ngBootbox) {
        $scope.page = 0;
        $scope.pagesCount = 0;
        $scope.pageSize = 10;
        $scope.totalCount = 0;
        ...
        function getListProduct(page) {
            apiService.get("http://localhost:5260/api/Product/Clothe/GetListPaging/" + page + "/" + $scope.pageSize + "/" + $scope.searchKey, null, function (result) {
                $scope.page = page;
